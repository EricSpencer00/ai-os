...
# Configurable runtime user for the VM (defaults to the common Multipass image user)
...
USER_HOME="/home/${AURAOS_USER}"
AGENT_DIR="/opt/auraos/gui_agent"

echo "[gui-bootstrap] Configuring VNC xstartup for user ${AURAOS_USER}"
...
chown -R ${AURAOS_USER}:${AURAOS_USER} "$USER_HOME/.vnc"

# Create a VNC password for the ubuntu user so tigervnc can start non-interactively
echo "[gui-bootstrap] Setting VNC password for ${AURAOS_USER} user"
mkdir -p "$USER_HOME/.vnc"
printf "auraos123\n" | vncpasswd -f > "$USER_HOME/.vnc/passwd" || true
chown ${AURAOS_USER}:${AURAOS_USER} "$USER_HOME/.vnc/passwd" || true
...
Type=simple
User=root
ExecStart=/bin/bash -lc 'Xvfb :1 -screen 0 1280x720x24 & sleep 1; export DISPLAY=:1; su - ${AURAOS_USER} -c "xauth generate :1 . trusted || true"; chown ${AURAOS_USER}:${AURAOS_USER} /home/${AURAOS_USER}/.Xauthority >/dev/null 2>&1 || true; su - ${AURAOS_USER} -c "dbus-launch startxfce4" & sleep 1; /usr/bin/x11vnc -auth /home/${AURAOS_USER}/.Xauthority -display :1 -rfbport 5900 -rfbauth /home/${AURAOS_USER}/.vnc/passwd -forever -shared -noxdamage -nowf -ncache 10'
Restart=on-failure
...
if [ ! -d /opt/novnc ]; then
    ...
    chown -R ${AURAOS_USER}:${AURAOS_USER} /opt/novnc || true
fi
