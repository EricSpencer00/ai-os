...
if multipass exec auraos-multipass -- pgrep -x -u ${AURAOS_USER:-ubuntu} xfce4-session >/dev/null 2>&1; then
...
if multipass exec auraos-multipass -- pgrep -x -u ${AURAOS_USER:-ubuntu} xfwm4 >/dev/null 2>&1; then
  echo "✓ xfwm4 running"
else
...
if multipass exec auraos-multipass -- pgrep -x -u ${AURAOS_USER:-ubuntu} xfce4-panel >/dev/null 2>&1; then
